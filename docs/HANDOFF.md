...
Last updated: 2026-07-05 by Claude Code (Phase 8c-r1 closed: valify-scope and parse-error recovery complete, Phase 8 fully complete).
...
- Groq Arabic quality test: run classify_batch on a 20-row sample of Egyptian Arabic reviews using GroqProvider directly. Compare valify_scope and sentiment output against Gemini output for the same rows. If accuracy is acceptable, Groq is confirmed as production fallback. If not, evaluate Ollama with Qwen2.5 or Aya-23.
...
- Ollama deferred: only revisit if Groq Arabic quality test fails. Local model hosting adds infrastructure complexity not justified at current scale.
